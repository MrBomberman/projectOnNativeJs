    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // этот блок создается только в момент отправки
...
function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  var content = document.querySelectorAll(contentSelector); // чтобы получить сразу все контенты
  // функция скрывает весь контент и убирает класс активности у ненужных табов

  function hideTabContent() {
    // здесь лежит коллекция контента
  ...
  header.addEventListener('click', function (e) {
    // обработчик события на область со всеми табами
    var target = e.target; // тот элемент, куда кликнул пользователь

    if (target && target.classList.contains(tabSelector.replace(/\./, '')) || // находим точку и меняем ее на пустое поле -
    target.parentNode.classList.contains(tabSelector.replace(/\./, ''))) {
      // мы можем кликнуть не только в элемент по селектору, но и во внутрь его, элемент который будет относится к нашему селектору
      // проверяем куда кликнул пользователь при помощи класса таба
      tab.forEach(function (item, i) {
        // перебираем все табы с индексом
        if (target == item || target.parentNode == item) {
          // если мы кликнули в элемент, который сейчас перебирается
          hideTabContent();
          showTabContent(i);
        }
      });
